...
const DropDownMenu = ({ handleDropDownClick, from}) => {
    ...
    >
      <Menu.Items className="bg-white absolute right-0 z-10 w-40 origin-top-right rounded-md  shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
        <div className="py-1">
          <Menu.Item>
              <div  onClick={(e) => {
                if(from == "screenGroupTable"){
                 
                }
                else{
                  handleDropDownClick('resend')
                }
              }} className={`flex flex-row items-center cursor-pointer p-2 hover:bg-[#0000000A] ${from == "screenGroupTable"  ? 'justify-center' : 'justify-end'}`}>
                {
                  from == "screenGroupTable" ? <p className="font-medium">Ungroup</p> : <><LiaShareSquareSolid className='me-1'  size={20}/>
          ...
          <Menu.Item>
               <div onClick={(e) => {
                  if(from == "screenGroupTable"){
                    handleDropDownClick('changeGroup')
                  }
                  else{
                ...
                {
                  from == "screenGroupTable" ?  <p className="font-medium">Change Group</p> : <><MdOutlineDelete color='red' className='me-2' size={24}/>
